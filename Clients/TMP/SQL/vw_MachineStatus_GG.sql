...
WITH JobCompletionPredictions AS (
    SELECT 
        jo.Job_Operation,
        jo.Job,
        jo.Work_Center,
...
MachineStatusCTE AS (
    -- PART 1: ORIGINAL WORKING VIEW LOGIC + MINIMAL SETUP ADDITIONS
    SELECT DISTINCT
            ...
            -- Individual Status Fields - ENHANCED to handle setup statuses
            CASE 
                WHEN current_op.Status = 'C' THEN 
                    CASE 
                        WHEN setup_for_machine.setup_status = 'ACTIVE SUP' THEN 'STAGING'
        ...
        SELECT 
            ...
            -- MINIMAL SETUP ADDITION: Machine status with setup
            CASE 
...
FROM (
    SELECT *,
        ...
        MAX(CASE WHEN MachineStatus IN ('ACTIVE', 'IDLE', 'OverPlan', 'STAGING', 'STAGING-IDLE', 'STAGED') THEN 1 ELSE 0 END) 
            OVER (PARTITION BY Machine) AS has_active_work,
        -- Rank OFFLINE records (keep best one if no active work)
        ROW_NUMBER() OVER (
            PARTITION BY Machine, CASE WHEN MachineStatus = 'OFFLINE' THEN 1 ELSE 0 END
