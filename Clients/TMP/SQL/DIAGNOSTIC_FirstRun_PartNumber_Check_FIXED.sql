-- ========================================================================
-- FIRST RUN DIAGNOSTIC - Check Part Number History (COMPATIBLE VERSION)
...
SELECT 
    ...
    -- Show which previous jobs had production (SQL 2008 compatible)
    STUFF((
        SELECT ', ' + CAST(prev_jobs.Job AS VARCHAR(20))
        FROM (
        ...
        ORDER BY prev_jobs.Job
        FOR XML PATH('')
    ), 1, 2, '') AS Previous_Jobs_With_Production
...
SELECT 
    ...
    -- Show the reason (SQL 2008 compatible)
    CASE 
        WHEN NOT EXISTS (
            SELECT 1 
            FROM THURO.dbo.Job j with (NoLock)
        ...
        WHEN EXISTS (
            ...
            WHERE j_curr.Part_Number = @PartNumber
              AND j_curr.Status NOT IN ('Complete', 'Closed', 'Shipped')
              AND jot_hist.Act_Run_Qty > 0
        ) THEN 'Previous job(s) with production exist: ' +
            ISNULL(STUFF((
                SELECT ', ' + CAST(prev_jobs.Job AS VARCHAR(20))
                FROM (
                    SELECT DISTINCT j_prev.Job
